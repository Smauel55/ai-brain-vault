...
- **Page-break rule (2026-06-13):** `sec()` wraps each section in one `KeepTogether` so a section never splits across a page; if it will not fit in the space left it starts on the next page. The article opts out (`keep_whole=False`) so it flows. GOTCHA baked into `sec()`: inner atomic blocks are pre-wrapped `KeepTogether`, whose `wrap()` returns a sentinel `0xffffff` height; nesting them made every section land on its own page (3->7 pp), so `sec()` flattens the inner blocks before the outer wrap. `build()` signature unchanged, so the workflow import is unaffected.
