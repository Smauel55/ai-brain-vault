...
- **Built (2026-06-11):** BROUGHT FORWARD from fast-follow and built same day. Content side: [[General-English-Mode-Spec]] (D11) defines the audience dimension + AP->Common Core skill map; `render_opener_v2.py` parameterized with an `audience` field (`ap_lang` default | `general_english`) swapping masthead tagline, MCQ header, writing-type label, and the alignment section; dual-write (`gen_alignment` / `gen_type`) wired into the sample prompt + MCQ spec. Verified by rendering the same piece both ways (labels swap, pedagogical content identical). Page side: drove Base44; added an `audience` field to the Teacher entity (lock intact `{create/read/update/delete:false}`), threaded it through `getTeacherByToken` (returns it, default ap_lang) + `updateTeacherByToken` (validates + persists via service role), and added a prefilled "Course" radio group to /manage. Code-verified in PREVIEW; awaiting Samuel's Publish + post-publish live re-verify.
- **Status:** BUILT in preview (content + page). Open: Samuel publishes the Base44 change; then live re-verify (leak probe still `[]`, /manage prefills + saves audience). Reading-level fit with a general-English teacher still to confirm before marketing the expansion.
