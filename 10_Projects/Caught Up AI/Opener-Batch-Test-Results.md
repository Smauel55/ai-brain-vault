...
> **Issue 3 DONE 2026-06-08.** #2 and #4 hand-tagged (11 spans from the device purposes); all 28 openers now fully tagged, citation-free, render-clean (full render-truth crosscheck passes).
> **DEVICE-LABEL CLEANUP DONE 2026-06-08 (made permanent).** Systematic audit of all ~145 device labels across 28 openers against [[Rhetorical-Device-Vocabulary]]. Found 4 off-list labels + 1 wrong device + 2 non-devices (the eyeball pass had only caught 2). Fixes via `fix_openers_labels.py`: relabel #1 Statistical evidence->Logos, #2 Periodic sentence->Cumulative and periodic sentences, #4 Polysyndeton->Asyndeton, #26 Redefinition->Antithesis; DROP #2 "Charge / obligation" (register close-type, not a device) and #17 "Syntax" (inert aside) -- both were the last device, no renumbering. BORDERLINE/override candidate: #4 dev3 "Allusion"->Logos (verbatim constitutional clause as the legal premise; Allusion was wrong but Ethos/Logos/keep all arguable -- Samuel can override). Kept: the four Diction labels (genuine word-choice) and #11 Tone (defensible). Permanent fix in `wf_generate.js`: deterministic `deviceLint` rejects any off-list label (forces redraft; tested -- flags the 4 raw off-list labels, passes the fix), devices verifier strengthened (umbrella last-resort, asyndeton/polysyndeton conjunction count, allusion-vs-quotation, no register-close-types), draft prompt reinforced, and [[Rhetorical-Device-Vocabulary]] updated with the banned variants + the enforcement note. REMAINING: #24 residual study-description; the unspecified "article type" issue (needs Samuel); cross-piece sameness critic over the 28; accuracy spot-checks (#23 Boeing, political Open Letters).
