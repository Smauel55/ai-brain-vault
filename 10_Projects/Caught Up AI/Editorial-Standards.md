...
**Pointed mode (CIVIC tier only): issue advocacy.** On the "includes civic / current events" tier, an Opener *may* take a pointed position and argue it with full force, because AP Lang teaches persuasion and position-taking rhetoric is the richest material to analyze. This is allowed under all of the following conditions:

- **A clear rhetorical situation, but the FORM varies across the batch.** What's unlocked is the *stance*, not one delivery format. Pointed pieces should spread across forms so they do not all feel like the same template: an op-ed or signed column in a newspaper, an unsigned editorial in the paper's own voice, a first-person personal essay, an open letter to a body, a witness account, a guest commentary. Do NOT default every pointed piece to "a named individual testifying to a group" — that sameness is its own AI tell (see [[Cross-Piece-Sameness-Rubric]], [[feedback_ai_writing_formulaic]]). The headnote still names the rhetorical situation the AP class analyzes (speaker or speaking voice, audience, occasion; see [[Headnote-Spec]]), and the speaker is framed by role or relationship to the issue, never given an invented proper name and never a real person. A collective or institutional voice (an editorial board) is fine; a single first-person voice is fine; an addressee-driven letter is fine. Vary it.
- **First-person or argued emotion is welcome.** Let the piece be angry, grieving, hopeful, indignant where the form supports it (an op-ed argues; a personal essay confides; an editorial presses). The move from stake to public argument is exactly the ethos/pathos the class mines. A bloodless position paper has nothing to analyze.
