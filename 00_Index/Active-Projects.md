...
- 🟢 [[Caught Up AI]] — AP Lang teacher daily-brief startup. *Status: spec v1.1 (D-002 script revision, D-003 scope cut); one-pager v0.4 paused; teacher meeting brief v0.2 with real sample "The Quiet Classroom" (2026-05-19); customer discovery ~May 25. Product work otherwise paused until ~May 25 (Claude mastery plan).*
  ...
  - Opener customization (2026-06-07): raised difficulty + political-intensity customization. Recommendation (not yet confirmed by Samuel): treat the two axes differently. Political intensity = a trust/safety gate, keep it but as a single low-cardinality SETTING at signup (~2 tiers, default neutral), implemented as topic routing not a second pipeline. Difficulty = effectively a separate product, pick one lane (AP Lang) at launch and defer tiers to v2. Reject the 5-10/day buffet (breaks the print-and-go ritual + multiplies QA, the real bottleneck under AI-generation). Optional cheap low-level support = a scaffolding layer (glossary/sentence-starters) on the same passage. Detail in [[2026-06-07 - Opener customization strategy]].
